...
- [x] store the HK holidays in database
- [x] output the holiday list from database
- [x] filter & re-output the holiday list from database


### Setup

- Start MySQL database in localhost
- Create an empty  `hk_holidays` batabase ()
```sql
CREATE DATABASE `hk_holidays`;
```
- Setup your username and password in class ``
```java
...
Utilised framework JUnit to perform the unit testing for controller, parsing Json and implementing database.
...
<img src="pic/2022-11-03_132902.png" width="720">
...
- Design basic GUI with `home.html`

- Built a class `HomeController` Utilising `spring` framework to construct web bind
    - a `GetMapping` for getting the home page 
    - a `GetMapping` for getting data from database as data model

- Built a class `Holday` for Java `MVC` design pattern

- Built a `JsonHandler` class to parse Json file
...
- Implement database `MySQL` with `java.sql` and `JDBC` driver
...
- Desined UI using CSS stylesheet
...
- Test the Controller for web app
